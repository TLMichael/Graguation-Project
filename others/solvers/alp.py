...
class Solver_ALP(object):
    """Solver for adversarial training with logit pairing"""
    ...
    def __init__(self, train_loader, val_loader, config):
        ...
        self.postfix_tag = '{}/ALP'.format(self.dataset.lower())
    ...
    def get_alp_loss(self, logits_cln, logits_adv):
        diff = logits_adv - logits_cln
    ...
    def epoch_train(self, model, opt, attack, **kwargs):
        ...
        for idx, (x, y) in self.train_loader:
            ...
            alp_loss = self.get_alp_loss(logits_cln, logits_adv)
            
            loss = (loss_cln + loss_adv) + self.lambda_item * alp_loss
